...
<form class="form" action="{{ route($routeName . '.update', $postData->id) }}" method="POST">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mb-2 form-group">
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title', $postData->title) }}" class="form-control" placeholder="Enter post title" required>
                    </div>
                    ...
                    <div class="mb-2 form-group">
                        ...
                        <textarea name="content" class="form-control text-editor" rows="4">{{ old('content', $postData->content) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mb-2 form-group">
                        ...
                        <input type="text" class="form-control" id="slug" name="slug" value="{{ old('slug', $postData->slug) }}" placeholder="Enter Slug" required>
                        @php
                            $autoSlugEnabled = old('auto_slug_enabled', !empty($postData->auto_slug));
                        @endphp
                        <div class="form-check mt-2 d-none">
                            <input class="form-check-input" type="checkbox" id="auto_slug_enabled" name="auto_slug_enabled" value="1" @checked($autoSlugEnabled)>
                    ...
                    <div class="mb-2 form-group d-none">
                        ...
                        <input type="text" class="form-control" id="language" name="language" value="{{ old('language', $postData->language ?? 'en') }}" placeholder="en">
                    </div>
                    ...
                    <div class="mb-2 form-group">
                        ...
                        <select name="author_id" class="form-select select2" required>
                            ...
                            @foreach ($authors as $author)
                                <option value="{{ $author->id }}" @selected(old('author_id', $postData->author_id) == $author->id)>{{ $author->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-2 form-group clearfix">
                        ...
                        <div class="input-group" data-toggle="aizuploader" data-type="image" data-multiple="false">
                            ...
                            <input type="hidden" name="featured_image" class="selected-files" value="{{ old('featured_image', $postData->featured_image) }}">
                        </div>
                    ...
                    <div class="mb-2 form-group clearfix">
                        ...
                        <div class="input-group" data-toggle="aizuploader" data-type="image" data-multiple="true">
                            ...
                            <input type="hidden" name="featured_detail_image" class="selected-files" value="{{ old('featured_detail_image', $postData->featured_detail_image) }}">
                        </div>
                        <div class="file-preview box sm"></div>
                    </div>
                    ...
                    <div class="mb-2 form-group">
                        ...
                        @php
                            $publishedAtValue = $postData->published_at ? \Carbon\Carbon::parse($postData->published_at)->format('Y-m-d\\TH:i') : '';
                        @endphp
                        <input type="datetime-local" class="form-control" name="published_at" value="{{ old('published_at', $publishedAtValue) }}">
                    </div>
                    ...
                    <div class="form-group mb-2 d-none">
                        ...
                        <select name="company_id" class="form-select select2" required>
                            @foreach (getCompanyList() as $row)
                                <option value="{{ $row->id }}" @if($postData->company_id == $row->id) selected @endif>
                                    {{ $row->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group mb-2">
                        ...
                        <select name="is_active" class="form-select select2" required>
                            <option value="1" @if($postData->is_active == 1) selected @endif>Active</option>
                            <option value="0" @if($postData->is_active == 0) selected @endif>Inactive</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mb-2 form-group">
                        ...
                        <input type="text" id="seo_title" name="seo_title" value="{{ old('seo_title', $postData->seo_title) }}" class="form-control" placeholder="Enter meta title">
                    </div>
                    <div class="mb-2 form-group">
                        <label for="seo_description" class="form-label">Meta Description</label>
                        <textarea class="form-control" id="seo_description" name="seo_description" rows="3" placeholder="Enter meta description">{{ old('seo_description', $postData->seo_description) }}</textarea>
                    </div>
                    {{-- <div class="mb-2 form-group">
                        <label for="seo_keywords" class="form-label">Meta Keywords</label>
                        <input type="text" id="seo_keywords" name="seo_keywords" value="{{ old('seo_keywords', $postData->seo_keywords) }}" class="form-control" placeholder="Enter meta keywords">
                    </div>
                    <div class="mb-2 form-group">
                        <label for="seo_schema" class="form-label">Schema Markup</label>
                        <textarea class="form-control" id="seo_schema" name="seo_schema" rows="3" placeholder="Enter schema markup">{{ old('seo_schema', $postData->seo_schema) }}</textarea>
                    </div> --}}
                </div>
            </div>
